...
@implementation MSGuideViewManager

+ (instancetype)sharedInstance {
    static MSGuideViewManager *instance = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        instance = [MSGuideViewManager new];
    });
    return instance;
}
...
- (UICollectionView *)view {
    if (!_view) {
        ...
        layout.itemSize = kMSGuideViewBounds.size;
        layout.scrollDirection = UICollectionViewScrollDirectionHorizontal;
        
        _view = [[UICollectionView alloc] initWithFrame:kMSGuideViewBounds collectionViewLayout:layout];
        _view.bounces = NO;
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    cell.imageView.center = CGPointMake(kMSGuideViewBounds.size.width / 2, kMSGuideViewBounds.size.height / 2);
