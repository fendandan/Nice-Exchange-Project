...
#define APPDELEGATE     [SWAppDelegate shareSWAppDelegate]  // (( *)[[UIApplication sharedApplication] delegate])
...
- (instancetype)init {
    if ((self = [super initWithFrame:CGRectMake(0, 0, Size, Size)])) {
        ...
        [self setAlpha:0];
        [self setCenter:APPDELEGATE.swWindow.center];
        ...
        [APPDELEGATE.swWindow addSubview:self];
    }
    return self;
}
...
+ (void)showWithOverlay {
    [self dismiss:^{
        [APPDELEGATE.swWindow addSubview:[[self instance] overlay1]];
        [self show];
    }];
}
...
+ (void)show {
    [self dismiss:^{
        [APPDELEGATE.swWindow bringSubviewToFront:[self instance]];
...
- (UIView *)overlay1 {
    ...
    if (!self.overlayView) {
        self.overlayView = [[UIView alloc] initWithFrame:APPDELEGATE.swWindow.frame];
